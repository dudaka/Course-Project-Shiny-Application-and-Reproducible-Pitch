...
shinyServer(function(input, output) {
  ...
  output$da.plot <- renderPlot({
    ...
    accuracy <- paste0(round(mean(sum(round(predictions) == da.dataset[,3]) / 3), 4), "%")
    # accuracy <- paste0(mean(as.double(predictions == da.dataset[,3])) * 100, "%")
